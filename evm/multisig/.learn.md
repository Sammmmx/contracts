...
Any signer can submit a transaction proposal to the contract. The transaction is stored in a `PENDING` state and assigned a unique ID.
...
Upon submission, the contract automatically records the submitter's approval via `_tryAutoConfirm`.
...
- **TxType:** Categorizes what the transaction intends to do (ETH, ERC20, etc.).
- **States:** Tracks if a transaction is `NONE`, `PENDING`, `EXECUTED`, or `CANCELLED`. Once `EXECUTED` or `CANCELLED`, a transaction cannot be run again, protecting against **Reentrancy** or **Double-Spend** attacks.
